...
export class LightService {
    ...
    /**
     * Creates a bell curve that peaks at the center value and falls off to zero
     */
    ...
    /**
     * Calculate a smooth night factor (0 during day, 1 at night)
     */
    private getNightFactor(time: number, sunrise: number, sunset: number): number {
        // Create larger transition windows
        const dayStart = sunrise - 0.5;  // Start transition before sunrise
        const dayEnd = sunset + 0.5;     // End transition after sunset

        // Handle day wrapping
        let wrappedTime = time;
        if (sunset < sunrise) {
            if (time < sunrise) wrappedTime += 24;
        }
